...
### Module Leads: Julio Diaz Caballero and Caterina Guzmán Verri
...
Add the project **Dublin 2026 K pneumoniae Outbreak** by clicking on `+ ADD PROJECT` and typing into the search box --you will need to be connected to WiFi.
...
Open the **Dublin 2026 K pneumoniae Outbreak** project and add an entry. Follow the form to collect a sample and associated metadata.
...
Only one member of each pair should do this. **Upload your entry (and image)** to the Epicollect5 server --make sure you are connected to the WiFi.

![](img/epicollect_upload_data.png)

Submit your sample to the instructor; it will be sent to the reference lab for culture confirmation.

Once all entries are uploaded by the disease detectives we will take a look at the data together on [**https://five.epicollect.net/project/dublin-2026-k-pneumoniae-outbreak**](https://five.epicollect.net/project/dublin-2026-k-pneumoniae-outbreak)
...
Drag and Drop provided fasta files ([file1](https://drive.google.com/drive/folders/10TqyS_APg3cDz68NvzMJ5Clc0c6H4dTY?usp=sharing) and [file2](https://drive.google.com/drive/folders/10TqyS_APg3cDz68NvzMJ5Clc0c6H4dTY?usp=sharing)) into the browser and wait for analysis to finish
...
You should now be able to find the [`mlst-Pasteur.csv`](https://drive.google.com/drive/folders/10TqyS_APg3cDz68NvzMJ5Clc0c6H4dTY?usp=sharing) in your `Downloads` folder.
...
Open the Data-flo workflow ([**https://www.data-flo.io/run/jPZFNdqdkixkRnyuDkH2ti**](https://www.data-flo.io/run/jPZFNdqdkixkRnyuDkH2ti))
...
Now let's go to the implementation page by clicking on the `RUN` option.
...
Change the colour column to "Source". Click on the eye icon at the top right of the page. Then select "Source" from the `Colour Column` dropdown list.

![](img/microreact_source.png)

Microreact assigns colours automatically to every column in your metadata table, but you can customise them. Let's change the colour column to "Infection Type". Click on the eye icon at the top right of the page. Then select "Infection Type" from the `Colour Column` dropdown list.
...
1. Can you confirm that the K. pneumoniae isolated from the CRAIC campus are closely related to the patient isolates?
2. Was it useful to include genomes from a previous outbreak in the analysis? Is the current outbreak related to the past outbreak from Mar-Apr 2024?
3. Is the CRAIC Campus the origin of the outbreak?
...
a.  Go to the [**epicollect link provided above**](https://five.epicollect.net/project/dublin-2026-k-pneumoniae-outbreak).
